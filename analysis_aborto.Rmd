...
tabla <- tabla %>%
  mutate(
    aborto_espontaneo = ifelse(tipo_fin == "spontaneous abortion", 1, 0),
...
riesgos <- tabla %>%
  ...
  summarise(
    total = n(),
    casos_aborto = sum(aborto_espontaneo),
    riesgo_absoluto = casos_aborto / total
  )
...
tabla_cont <- table(tabla$tratadas_p1, tabla$aborto_espontaneo)
...
vars = c('age','country','exclusion_risk',"m_smoking","m_alcohol","m_drug_abuse","m_diabetes","obesity_final","m_lipid",'m_depression',
         'm_anxiety',"m_psychotic",'prev_aborto')
...
ps_data <- gaia_embarazos_hiper_comp_inter %>%
  select(embarazo_id,abortion,tratadas_p1, all_of(vars)) %>%
  distinct(embarazo_id, .keep_all = TRUE)  #elimina duplicados embarazo (se queda con el primero)


formula_ajustada <- as.formula(paste("abortion ~ tratadas_p1 +", paste(vars, collapse = " + ")))
```


Cálculo de propensity scores:
```{r}
modelo_ps <- glm(tratadas_p1 ~ ., data = ps_data %>% select(-embarazo_id,-abortion), family = binomial)
...
```{r}
#ps_data <- ps_data %>%
 # mutate(tratadas_p1 = case_when(
  #  tratadas_p1 == "1" ~ "SI",
   # tratadas_p1 == "0" ~ "NO" # Mantiene los valores originales si no coinciden
#  ))
ggplot(ps_data, aes(x = pscore, fill = as.factor(tratadas_p1))) +
  geom_density(alpha = 0.4) +
  scale_fill_manual(values = c("steelblue", "firebrick")) +
  labs(
    title = "Distribución del Propensity Score por Grupo de tratados",
    x = "Propensity Score",
    y = "Densidad",
...
riesgos_ponderados <- ps_data %>%
  ...
  summarise(
    ...
    casos_cong_ponderados = sum(weight_iptw * abortion), # Casos ponderados
...
tabla_cont_ponderada <- svytable(~tratadas_p1 + abortion, design = design_ps)
...
tabla_modelo_iptw <- ps_data %>% 
  ...
  summarise(
    ...
    casos  = sum(weight_iptw * abortion),      # Abortos ponderados
...
match_ps <- matchit(tratadas_p1 ~ m_smoking + m_alcohol + m_drug_abuse +  m_lipid + m_diabetes + 
                      obesity_final  + age + exclusion_risk + country + m_depression + m_anxiety + m_psychotic + prev_aborto,
                    data = ps_data,
...
tabla_cont_macth <- table(matched_data$tratadas_p1, matched_data$abortion)
...
```{r}
modelo_ajustado <- glm(abortion ~ tratadas_p1 + m_smoking + m_alcohol + m_drug_abuse +  m_lipid + m_diabetes + 
                      obesity_final  + age + exclusion_risk + country + m_depression + m_anxiety + m_psychotic + prev_aborto,
...
tabla_modelo_match <- matched_data %>% 
  ...
  summarise(
    ...
    casos  = sum(abortion),                  # Número real de abortos
...
gaia_embarazos_disp_ten_hiper <- gaia_embarazos_disp_ten_hiper %>% mutate(abortion = if_else(tipo_fin=='spontaneous abortion',1,0))
...
vars = c('age','country','exclusion_risk',"m_smoking","m_alcohol","m_drug_abuse","m_diabetes","obesity_final","m_lipid",'m_depression',
         'm_anxiety',"m_psychotic",'hipertension_post','prev_aborto')
#la depresión y la epilepsia también salían importantes para separar tratadas de no tratadas
...
roc_obj <- roc(ps_data$abortion, probabilidades)
...
Para abortos:
```{r}
#gaia_embarazos_hiper_comp = gaia_embarazos_hiper_comp %>% filter(imputado==0)
```
...
general <- as.data.frame(rr_resultado$measure)[2, ]  # Fila 2 contiene el RR para el grupo tratado
p_value <- as.data.frame(rr_resultado$p.value)[2, ]  # Fila 2 contiene el p-value para el grupo tratado

# Unir ambas en una sola tabla
general <- cbind(general, p_value)
...
creamos la base de datos para los análisis:
```{r}
vars=c('age','country','exclusion_risk',"m_smoking","m_alcohol","m_drug_abuse","m_diabetes","value_imc","m_lipid",'m_depression',
         'm_anxiety',"m_psychotic",'prev_aborto')
...
tabla_comparativa1 %>%
  kable(
    format = "html",
    caption = "Comparación de Métodos para Odds Ratio en el riesgo de aborto espontáneo",
    col.names = c("Método", "Grupo", "N", "Casos", "Tasa", "OR (IC 95%)"),
    align = "lccccl"
  ) %>%
...
lim_inf <- min(tratadas$LCI, na.rm = TRUE) * 0.8   # un 20 % por debajo
lim_sup <- max(tratadas$UCI, na.rm = TRUE) * 1.2   # un 20 % por encima
...
ggplot(tratadas, aes(x = OR, y = orden)) +
  ...
  ylab("Método")  +
  scale_x_continuous(
    limits = c(0.5, 1.5),  # Ajustando el límite superior a 1.5
    breaks = c(0.7, 0.8, 0.9, 1, 1.1, 1.2,1.3,1.4,1.5),  # Incluir valores mayores a 1
    labels = label_number(accuracy = 0.01)) +
  ggtitle("") +
  theme_minimal() +
  theme(
